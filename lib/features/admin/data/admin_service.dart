...
class AdminService {
  ...
  /// Initialise les plans entreprise dans Firestore
  Future<void> seedCorporatePlans() async {
    final plans = [
      {
        'code': 'starter_pro',
        'type': 'enterprise',
    ...
    for (var planData in plans) {
      final docRef = _firestore.collection('plans').doc(planData['code'] as String);
      batch.set(docRef, {
        ...planData,
        'createdAt': FieldValue.serverTimestamp(),
    ...
    await batch.commit();
    await _logAction('SEED_PLANS', 'Corporate plans initialized/updated', {});
  }
